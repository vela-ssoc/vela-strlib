...
func NewAcMatcher(dictionary []string) *Matcher {
	m := new(Matcher)
...
func NewAcFile(filename string) (*Matcher, error) {
...
func (m *Matcher) MAE(input []byte) (ret []string) { //MatchAndExtract
	m.counter++
	Call(input, m.root, func(offset int, f *node) {
		if f.counter == m.counter {
			return
		}

		f.counter = m.counter // thread not safe
		ret = append(ret, string(f.b))
	})
	return
}

func (m *Matcher) Replace(input []byte, sub []byte) []byte {
	last := 0
	var buff bytes.Buffer

	Call(input, m.root, func(offset int, item *node) {
		size := len(item.b)
		if size == 0 {
			return
		}

		field := input[last : offset-size+1]
		buff.Write(field)
		buff.Write(sub)
		last = offset + 1
	})
	buff.Write(input[last:])
	return buff.Bytes()
}
...
func Call(in []byte, n *node, callback func(int, *node)) {
